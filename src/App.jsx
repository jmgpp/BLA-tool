...
function App() {
  ...
  const [categories, setCategories] = useState([
    { id: 1, name: 'Medical', order: 1, subcategories: [
      { id: 1, name: 'Conditions & Symptoms' },
      { id: 2, name: 'Procedures & Tests' },
      { id: 3, name: 'Anatomy & Body Parts' },
      { id: 4, name: 'Medical Equipment & Devices' },
      { id: 5, name: 'Medications & Treatment' },
      { id: 6, name: 'Pregnancy & Childbirth' },
      { id: 7, name: 'Medical Actions & Processes' }
    ]},
    { id: 2, name: 'Legal', order: 2, subcategories: [
      { id: 8, name: 'Court Proceedings' },
      { id: 9, name: 'Documentation' }
    ]},
    { id: 3, name: 'Financial', order: 3, subcategories: [
      { id: 10, name: 'Banking' },
  ...
  // Load terms from the JSON file when component mounts
  useEffect(() => {
  ...
  const [modalAction, setModalAction] = useState('add'); // 'add', 'edit', 'delete'
  const [modalType, setModalType] = useState('category'); // 'category', 'subcategory', 'term'
  const [formName, setFormName] = useState('');

  // Modal states and form data for terms
  ...
  // Handle adding tags to the term form
  const handleTagInput = (e) => {
    if (e.key === 'Enter' || e.key === ',') {
      e.preventDefault();
      const value = e.target.value.trim();
      if (value && !termForm.tags.includes(value)) {
        setTermForm({
          ...termForm,
          tags: [...termForm.tags, value]
        });
        e.target.value = '';
      }
    }
  ...
  // Handle removing a tag from the term form
  ...
  const handleExportData = () => {
    // Create an object with all the data to export
    const exportData = {
      categories,
      terms,
      brands,
    ...
    const exportFileDefaultName = `interpreters_companion_data_${new Date().toISOString().slice(0, 10)}.json`;
  ...
  // Handle showing the import modal
    ...
    try {
      ...
      if (importData.categories) {
        if (importOption === 'replace') {
          ...
          // Merge approach - add new categories and update existing ones
          const mergedCategories = [...categories];
          importData.categories.forEach(importCat => {
            const existingCatIndex = mergedCategories.findIndex(cat => cat.id === importCat.id);
            if (existingCatIndex >= 0) {
              // Update existing category
              mergedCategories[existingCatIndex] = {
                ...mergedCategories[existingCatIndex],
                name: importCat.name,
                order: importCat.order
              };
              
              // Merge subcategories
              importCat.subcategories.forEach(importSubcat => {
                const existingSubcatIndex = mergedCategories[existingCatIndex].subcategories.findIndex(
                  subcat => subcat.id === importSubcat.id
                );
                if (existingSubcatIndex >= 0) {
                  mergedCategories[existingCatIndex].subcategories[existingSubcatIndex] = importSubcat;
                } else {
                  mergedCategories[existingCatIndex].subcategories.push(importSubcat);
                }
              });
            } else {
              // Add new category
      ...
      if (importData.terms) {
        if (importOption === 'replace') {
          setTerms(importData.terms);
        } else {
          // Merge approach - add new terms and update existing ones
          const mergedTerms = [...terms];
          importData.terms.forEach(importTerm => {
            const existingTermIndex = mergedTerms.findIndex(term => term.id === importTerm.id);
            if (existingTermIndex >= 0) {
              // Update existing term
              mergedTerms[existingTermIndex] = importTerm;
            } else {
              // Add new term
              mergedTerms.push(importTerm);
            }
          });
      ...
      // Process brands
      if (importData.brands) {
        if (importOption === 'replace') {
          setBrands(importData.brands);
        } else {
          // Merge approach for brands
          const mergedBrands = [...brands];
          importData.brands.forEach(importBrand => {
            const existingBrandIndex = mergedBrands.findIndex(brand => brand.id === importBrand.id);
            if (existingBrandIndex >= 0) {
              mergedBrands[existingBrandIndex] = importBrand;
            } else {
              mergedBrands.push(importBrand);
            }
          });
      ...
      // Process brand types
      if (importData.brandTypes) {
        if (importOption === 'replace') {
          setBrandTypes(importData.brandTypes);
        } else {
          // Merge approach for brand types
          const mergedBrandTypes = [...brandTypes];
          importData.brandTypes.forEach(importType => {
            const existingTypeIndex = mergedBrandTypes.findIndex(type => type.id === importType.id);
            if (existingTypeIndex >= 0) {
              mergedBrandTypes[existingTypeIndex] = importType;
            } else {
              mergedBrandTypes.push(importType);
            }
          });
          
          setBrandTypes(mergedBrandTypes);
        }
      }
  ...
  return (
    <div className="app-container">
      ...
      <Navbar bg="dark" variant="dark" expand="lg" className="mb-3">
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            ...
            <Nav>
              <Button variant="outline-light" className="me-2" onClick={handleShowImportModal}>
                Import
              </Button>
              <Button variant="outline-light" onClick={handleExportData}>
                Export
              </Button>
            </Nav>
          </Navbar.Collapse>
              ...
              <Card.Body>
                ...
                <Row className="mb-3">
                  <Col>
                    <InputGroup>
                      <Form.Control
                        placeholder="Search terms in English, Spanish, or by tags..."
                        value={searchTerm}
      ...
      <Modal show={showTermModal} onHide={() => setShowTermModal(false)}>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group controlId="termNotes">
            ...
            <Form.Group controlId="termTags" className="mt-3">
              <Form.Label>Tags (Optional) - Type and press Enter or comma to add</Form.Label>
              <Form.Control 
                type="text" 
              ...
              />
              <div className="mt-2">
                {termForm.tags && termForm.tags.map((tag, index) => (
                  <Badge 
                    key={index} 
                    bg="secondary" 
      ...
      <footer className="text-center mt-4 py-3 bg-light">
        <p>BLA-tool by Juan &copy; {new Date().getFullYear()}</p>
      </footer>
      ...
      <Modal show={showImportModal} onHide={() => setShowImportModal(false)}>
        ...
        <Modal.Body>
          <p>Upload a JSON file to import terms and categories.</p>
          <p>Choose how to handle the imported data:</p>
          <Form.Group controlId="importOption" className="mb-3">
            <Form.Check
              type="radio"
              name="importOption"
              id="merge"
              label="Merge with existing data (keep existing entries)"
              value="merge"
            ...
            <Form.Check
              ...
              name="importOption"
              id="replace"
              label="Replace existing data (caution: this will remove all current data)"
              value="replace"
              checked={importOption === 'replace'}
              onChange={(e) => setImportOption(e.target.value)}
            />
          </Form.Group>
          <Form.Group controlId="importFile" className="mt-3">
      ...
      <Modal show={showAboutModal} onHide={() => setShowAboutModal(false)}>
        ...
        <Modal.Body>
          <p>Developed by Juan Manuel Ganancias with Cursor AI and Claude 3.7</p>
